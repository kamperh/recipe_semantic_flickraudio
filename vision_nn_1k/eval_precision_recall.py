...
def eval_average_precision(word_to_id, n_most_common, sigmoid_output_dict, true_dict, show_plot=False):
    ...
    true_bow_vectors = np.zeros((len(sigmoid_output_dict), n_most_common), dtype=np.int)
    sigmoid_bow_vectors = np.zeros((len(sigmoid_output_dict), n_most_common))
    for i_utt, utt in enumerate(sorted(sigmoid_output_dict)):
        for word in true_dict[utt]:
            if word in word_to_id and word_to_id[word] < n_most_common:
                true_bow_vectors[i_utt, word_to_id[word]] = 1
        sigmoid_bow_vectors[i_utt, :] = sigmoid_output_dict[utt]
...
def main():
    ...
    word_to_id_fn = options_dict["word_to_id_fn"]
    ...
    captions_fn = path.join(data_dir, "captions.txt")
    ...
    for image_key in image_keys:
        for i_caption in xrange(5):
            ...
            if not label_key in captions_dict:
                print "Warning: Missing label " + label_key
            else:
                if not image_key in true_dict:
                    true_dict[image_key] = []
                for word in captions_dict[label_key]:
                    if word in word_to_id and word_to_id[word] < options_dict["n_most_common"]:
                        true_dict[image_key].append(word)
                # true_dict[image_key].extend(
                #     [word for word in captions_dict[label_key] if
                #     word_to_id[word] < ]
                #     )
